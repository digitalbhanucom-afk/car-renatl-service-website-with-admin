      ...
      <div className="container relative z-10 grid lg:grid-cols-12 gap-10 py-20">
        <div className="lg:col-span-8 animate-fade-in-up">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-primary/10 border border-primary/30 backdrop-blur-sm mb-6">
            ...
            <span className="text-xs uppercase tracking-widest font-semibold text-primary">{data?.hero_eyebrow ?? "Available 24/7 — Book Anytime"}</span>
          </div>

          <h1 className="font-display font-bold text-5xl sm:text-6xl lg:text-7xl xl:text-8xl leading-[0.95] mb-6">
            {titleParts[0]}<span className="text-gradient">{highlight}</span>{titleParts[1]}
          </h1>
          ...
          <div className="flex flex-wrap gap-3 mb-12">
            <a href={`tel:${phone}`} className="inline-flex items-center gap-2 px-7 h-14 rounded-full bg-gradient-primary text-primary-foreground font-semibold text-base btn-glow hover:scale-105 transition-transform">
              <Phone className="w-5 h-5" /> Call to Book
            </a>
            <a href={`https://wa.me/${wa}?text=${waMsg}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-7 h-14 rounded-full bg-secondary/80 backdrop-blur border border-border font-semibold text-base hover:bg-secondary transition-colors">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 max-w-2xl">
            {[
              { v: data?.rating ?? "4.9★", l: "Google Rating" },
              { v: data?.reviews_count ?? "240+", l: "Verified Reviews" },
              { v: data?.years_in_business ?? "8+", l: "Years in Business" },
        ...
        <div className="lg:col-span-4 hidden lg:flex flex-col gap-4 justify-center">
          {[
            ...
            { icon: Star, t: `Loved by ${data?.reviews_count ?? "240+"} Drivers`, d: `${data?.rating ?? "4.9★"} on Google` },
          ].map((f, i) => (
            <div key={i} className="card-elevated rounded-2xl p-5 border border-border/60 backdrop-blur animate-fade-in-up" style={{ animationDelay: `${0.2 + i * 0.15}s`, opacity: 0 }}>
